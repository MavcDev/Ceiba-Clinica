package com.ceiba.solicitudcita.entidad;
...
import static com.ceiba.solicitudcita.modelo.entidad.SolicitudCita.*;
import static org.junit.jupiter.api.Assertions.assertEquals;

class SolicitudCitaTest {

    @Test
    @DisplayName("Deberia crear correctamente la solicitud cita")
    void deberiaCrearCorrectamenteLaSolicitudCita() {
        SolicitudCita solicitudCita = new SolicitudCitaTestDataBuilder().conId(1L).build();
        assertEquals(1L, solicitudCita.getId());
        assertEquals(1L, solicitudCita.getMedico().getId());
        assertEquals(1L, solicitudCita.getUsuario().getId());
        assertEquals(1L, solicitudCita.getEspecialidad().getId());
    ...
    void deberiaFallarFechaCitaEsSabadoODomingoDeSolicitudCita() {
        LocalDate fechaCita = LocalDate.from(LocalDate.of(2022, 4, 10));
        SolicitudCitaTestDataBuilder solicitudCitaTestDataBuilder = new SolicitudCitaTestDataBuilder().conFechaCita(fechaCita);
